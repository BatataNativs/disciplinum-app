...
class SpendingModuleRepository implements ModuleRepositoryContract<SpendingModuleState> {
  ...
  Future<SpendingModuleState?> loadLocal(String userId) async {
    try {
      ...
      final isar = IsarService.instance.database;
      final entity = await isar.spendingGamificationEntitys.filter().userIdEqualTo(userId).findFirst();
  ...
  Future<bool> existsLocal(String userId) async {
    try {
      final isar = IsarService.instance.database;
      final count = await isar.spendingGamificationEntitys.filter().userIdEqualTo(userId).count();
      return count > 0;
    } catch (e) {
      return false;
    }
  ...
  Future<void> deleteLocal(String userId) async {
    try {
      ...
      final isar = IsarService.instance.database;
      await isar.writeTxn(() async {
        final entities = await isar.spendingGamificationEntitys.filter().userIdEqualTo(userId).findAll();
        await isar.spendingGamificationEntitys.deleteAll(entities.map((e) => e.id).toList());
      });
