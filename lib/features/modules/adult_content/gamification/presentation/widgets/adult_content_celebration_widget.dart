...
import 'package:disciplinum/features/modules/adult_content/gamification/domain/services/adult_content_celebration_service.dart';

/// Widget que escuta eventos de celebração do Adult Content e mostra UI de comemoração
class AdultContentCelebrationWidget extends StatefulWidget {
  final Widget child;

  const AdultContentCelebrationWidget({
    super.key,
    required this.child,
  });

  @override
  State<AdultContentCelebrationWidget> createState() => _AdultContentCelebrationWidgetState();
}

class _AdultContentCelebrationWidgetState extends State<AdultContentCelebrationWidget> {
  ...
  void _setupListeners() {
    ...
    _insigniaSubscription = EventBus.instance.stream
        .where((event) => event is AdultContentInsigniaConquistadaEvent)
        .listen((event) {
      final insigniaEvent = event as AdultContentInsigniaConquistadaEvent;
    ...
    _medalhaSubscription = EventBus.instance.stream
        .where((event) => event is AdultContentMedalhaConquistadaEvent)
        .listen((event) {
      final medalhaEvent = event as AdultContentMedalhaConquistadaEvent;
      _showMedalhaCelebration(medalhaEvent.medalhaId, medalhaEvent.medalhaName);
    });

    LoggerService.instance.gamification('🎉 AdultContentCelebrationWidget: listeners configurados');
  }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Você está mantendo o controle! Continue protegendo seu bem-estar digital.',
              textAlign: TextAlign.center,
              style: TextStyle(
                color: Colors.grey,
                fontSize: 14,
