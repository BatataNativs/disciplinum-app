...
import 'package:disciplinum/features/modules/diet/gamification/domain/services/diet_celebration_service.dart';

/// Widget que escuta eventos de celebração do Diet e mostra UI de comemoração
class DietCelebrationWidget extends StatefulWidget {
  final Widget child;

  const DietCelebrationWidget({
    super.key,
    required this.child,
  });

  @override
  State<DietCelebrationWidget> createState() => _DietCelebrationWidgetState();
}

class _DietCelebrationWidgetState extends State<DietCelebrationWidget> {
  ...
  void _setupListeners() {
    _insigniaSubscription = EventBus.instance.stream
        .where((event) => event is DietInsigniaConquistadaEvent)
        .listen((event) {
      final insigniaEvent = event as DietInsigniaConquistadaEvent;
    ...
    _medalhaSubscription = EventBus.instance.stream
        .where((event) => event is DietMedalhaConquistadaEvent)
        .listen((event) {
      final medalhaEvent = event as DietMedalhaConquistadaEvent;
      _showMedalhaCelebration(medalhaEvent.medalhaId, medalhaEvent.medalhaName);
    });

    LoggerService.instance.gamification('🎉 DietCelebrationWidget: listeners configurados');
  }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Você está cuidando da sua saúde! Continue mantendo hábitos saudáveis.',
              textAlign: TextAlign.center,
              style: TextStyle(
                color: Colors.grey,
                fontSize: 14,
