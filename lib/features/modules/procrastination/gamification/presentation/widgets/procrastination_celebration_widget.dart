...
import 'package:disciplinum/features/modules/procrastination/gamification/domain/services/procrastination_celebration_service.dart';

/// Widget que escuta eventos de celebração do Procrastination e mostra UI de comemoração
class ProcrastinationCelebrationWidget extends StatefulWidget {
  final Widget child;

  const ProcrastinationCelebrationWidget({
    super.key,
    required this.child,
  });

  @override
  State<ProcrastinationCelebrationWidget> createState() => _ProcrastinationCelebrationWidgetState();
}

class _ProcrastinationCelebrationWidgetState extends State<ProcrastinationCelebrationWidget> {
  ...
  void _setupListeners() {
    _insigniaSubscription = EventBus.instance.stream
        .where((event) => event is ProcrastinationInsigniaConquistadaEvent)
        .listen((event) {
      final insigniaEvent = event as ProcrastinationInsigniaConquistadaEvent;
    ...
    _medalhaSubscription = EventBus.instance.stream
        .where((event) => event is ProcrastinationMedalhaConquistadaEvent)
        .listen((event) {
      final medalhaEvent = event as ProcrastinationMedalhaConquistadaEvent;
      _showMedalhaCelebration(medalhaEvent.medalhaId, medalhaEvent.medalhaName);
    });

    LoggerService.instance.gamification('🎉 ProcrastinationCelebrationWidget: listeners configurados');
  }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Você está sendo produtivo! Continue focado em suas tarefas.',
              textAlign: TextAlign.center,
              style: TextStyle(
                color: Colors.grey,
                fontSize: 14,
